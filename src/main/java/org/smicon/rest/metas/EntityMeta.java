...
public class EntityMeta
{
	...
	protected void init(Class<?> aEntityClass) throws Exception
	{
		this.modelClass = aEntityClass;
	...
	public boolean containsSimpleIds()
	{
		return this.idPathParameterMetas.size() > 0;
	}

	public boolean containsEmbeddedIds()
	{
	...
	private void deduceChildEntityStructure() throws Exception
	{
		...
		for (PropertyDescriptor desc : this.modelBeanInfo.getPropertyDescriptors())
		{
			...
			EntityMeta meta = Metas.getEntityMeta(desc.getPropertyType());

			if (meta == null)
			{
				Class<?> target = this.getEntityCollectionTargetEntity(this.modelClass, desc);
				if (target != null)
				{
	...
	private Class<?> getEntityCollectionTargetEntity(Class<?> aParentType, PropertyDescriptor aPd) throws Exception
	{
		...
		try
		{
			field = this.modelClass.getDeclaredField(aPd.getName());
		}
		catch (Exception e)
		{};
	...
	private Class<?> getEntityCollectionTargetEntity(Class<?> aParentType, Class<?> aCollectionType) throws Exception
	{
		...
		return Metas.getEntityByName(targetClassName);
	}
	...
	private void checkAndPopulateSimpleIdField(Field aField) throws Exception
	{
		...
		{
			this.singularIdName = aField.getName();
			if (!Metas.isValidSimpleIdType(aField.getType()))
			{
				throw new IncorrectIdStructureException("The class: " + this.modelClass.getName() + " contains an incorrect id field: " + aField.getType()
	...
	private void checkAndPopulateSimpleIdAccessor(Method aAccessor) throws Exception
	{
		...
		if (id != null)
		{
			...
			if (!Metas.isValidSimpleIdType(aAccessor.getReturnType()))
			{
				throw new IncorrectIdStructureException("The class: " + this.modelClass.getName() + " contains an incorrect id accessor type: "
	...
	private PropertyDescriptor validateAndGetBeanPropertyDescriptor(BeanInfo aBeanInfo, String aPropertyName, Class<?> aPropertyType) throws Exception
	{
		...
		{
			if (Metas.validatePropertyDescriptor(pd, aPropertyName, aPropertyType))
			{
				return pd;
			}
		}
	...
	private void validateCompositeIdFields(Class<?> aCompositeIdType) throws Exception
	{
		...
		{
			if (!Metas.isValidSimpleIdType(field.getType()))
			{
	...
	private void validateContainsOnlyOneEmbeddeId() throws IncorrectIdStructureException
	{
		...
		{
			throw new IncorrectIdStructureException("The class: " + this.modelClass.getName() + " must define one and only one @EmbeddedId id!");
		}
	}
	...
	private void createEmbeddedKeyIdPathParameters()
	{
		...
		{
			if (!fieldNames.contains(field.getName()) && Metas.isAccessibleField(field))
			{
	...
	private void validateAndCreateEntityCompositeMapping(Class<?> aCompositeClass, String aEntityVariableName, String aCompositeVariableName) throws Exception
	{
		...
		{
			throw new IncorrectIdStructureException("The model entity id fields in the class: " + this.modelClass.getName()
